...
const rollbackTransaction = async (transaction: any) => {    
  ...
  // Rollback poin jika ada    
  if (transaction.pointsUse) {    
    await prisma.user.update({    
      where: { id: transaction.userId },    
      data: { point: { increment: transaction.pointsUse } },    
    });    
  }    
      
  // Rollback kupon jika ada    
...
// Fungsi untuk menjadwalkan tugas setiap 2 jam    
const scheduleRecurringTask = () => {    
  schedule.scheduleJob('checkTransactionsEvery2Hours', '0 */2 * * *', async () => {    
    const pendingTransactions = await prisma.transaction.findMany({    
      where: {    
        status: { in: ["waitingPayment", "waitingConfirmation"] },    
      },    
    ...
    await Promise.all(pendingTransactions.map(async (transaction) => {    
      if (transaction.status === "waitingPayment") {    
        await handleExpiration(transaction.id);    
      } else if (transaction.status === "waitingConfirmation") {    
        await handleCancellation(transaction.id);    
      }    
    }));    
  });    
...
export { loadJobs, scheduleTask, cancelTask, scheduleRecurringTask };    
